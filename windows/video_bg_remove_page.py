...
# ==========模型会话全局变量，先初始化为None，不提前加载==========
sess = None
remove = None # 新增全局remov
# ==============================================================
class VideoBgRemoveThread(QThread):
    ...
    def __init__(self, video_path, sample_step, mask_threshold=0.2, rembg_sess=None):
        super().__init__()
        ...
        self.rembg_sess = rembg_sess
    def run(self):
        if self.rembg_sess is None:
        ...
        while True:
            ...
            if idx % self.sample_step == 0:
                ...
                mask_rgba = remove(rgb, session=self.rembg_sess, alpha_matting=False, return_mask=True)
    ...
    def start_remove_bg(self):
        ...
        if sess is None:
            try:
                ...
                self.log(f"✅ 加载项目本地模型: {model_path}")
        ...
        self.thread = VideoBgRemoveThread(self.video_path, sample, mask_threshold=0.2, rembg_sess=sess)
